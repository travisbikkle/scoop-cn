...
Get-ChildItem -Recurse -Path .\bucket | ForEach-Object -Process {
    ...
    # DBeaver
    (Get-Content $_.FullName) -replace 'download\.dbeaver\.com/community', '$github_proxy/https://github.com/dbeaver/dbeaver/releases/download' | Set-Content -Path $_.FullName
    # Or
    ...
    # OBS Studio
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)\.zip', '$github_proxy/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1.zip' | Set-Content -Path $_.FullName
    # Or
    ...
    # OBS Studio 2.7
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)-Full', '$github_proxy/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1-Full' | Set-Content -Path $_.FullName
    ...
    # Strawberry
    (Get-Content $_.FullName) -replace 'files\.jkvinge\.net/packages/strawberry/StrawberrySetup-(.+)-mingw-x', '$github_proxy/https://github.com/strawberrymusicplayer/strawberry/releases/download/$1/StrawberrySetup-$1-mingw-x' | Set-Content -Path $_.FullName
    ...
    # FastCopy
    (Get-Content $_.FullName) -replace 'fastcopy\.jp/archive', '$github_proxy/https://raw.githubusercontent.com/FastCopyLab/FastCopyDist2/main' | Set-Content -Path $_.FullName
    ...
    # Scripts
    (Get-Content $_.FullName) -replace '(bucketsdir\\\\).+(\\\\scripts)', '$1easy-win$2' | Set-Content -Path $_.FullName

    # 将 depends 路径改为 easy-win
    (Get-Content $_.FullName) -replace '\"depends\":\s*\"(scoop\-cn/)?', '"depends": "easy-win/' | Set-Content -Path $_.FullName

    # 将 suggest 路径改为 easy-win
